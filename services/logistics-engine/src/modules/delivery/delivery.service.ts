...
export class DeliveryService {
  ...
  async updateStatus(orderId: string, status: DeliveryStatus, pin?: string) {
    ...
    if (
      ...
      (delivery as any).verificationPin
    ) {
      if (pin !== (delivery as any).verificationPin) {
        throw new Error(
          'Invalid verification PIN. Passenger must confirm delivery.',
        );
      }
    }
    ...
    if (status === DeliveryStatus.DELIVERED) {
      if (updatedDelivery.riderId) {
        ...
        const pickupTime = updatedDelivery.pickupTime
          ? new Date(updatedDelivery.pickupTime).getTime()
          : 0;
            ...
            .update({
              ...
              data: {
                reasons: {
                  ...(risk.reasons as any),
                  note: fraudAlert
                    ? 'Anti-Cheating threshold violated (Short trip)'
                    : 'Critical risk on delivery',
                },
              },
            })
