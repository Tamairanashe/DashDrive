...
export class NotificationsConsumer {
  ...
  async handleOrderStatusUpdate(@Payload() data: any) {
    this.logger.log(
      `Received status update for order ${data.orderNumber}: ${data.status}`,
    );

    const statusConfig = {
      PREPARING: {
        title: 'Order in Preparation',
        body: `Order #${data.orderNumber} is now being prepared.`,
      },
      READY: {
        title: 'Order Ready!',
        body: `Order #${data.orderNumber} is ready for pickup/delivery.`,
      },
      CANCELLED: {
  ...
  async handleInventoryLowStock(@Payload() data: any) {
    this.logger.warn(
      `Received ${PlatformEvent.LOW_STOCK_ALERT} for ${data.productName}`,
    );
    ...
    for (const device of devices) {
      await this.notificationsService.sendPush(device.pushToken!, title, body, {
