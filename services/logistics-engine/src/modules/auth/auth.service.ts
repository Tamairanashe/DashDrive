...
export class AuthService {
  ...
  async register(registerDto: RegisterDto) {
    try {
      const { merchant, store } =
        await this.merchantsService.register(registerDto);
      const payload = { sub: merchant.id, email: merchant.email };
      return {
        merchant,
        store,
  ...
  async forgotPassword(email: string) {
    ...
    if (!merchant) {
      ...
      return {
        message:
          'If that email is in our database, we will send a password reset link shortly.',
      };
    }

    this.logger.log(
      `Mocking password reset email send for legitimate user: ${email}`,
    );
    return {
      message:
        'If that email is in our database, we will send a password reset link shortly.',
