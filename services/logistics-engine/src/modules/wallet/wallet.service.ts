...
export class WalletService {
  ...
  async requestWithdrawal(walletId: string, amount: number) {
    ...
    if (risk.decision === RiskDecision.BLOCKED) {
      throw new BadRequestException(
        'Withdrawal blocked due to high security risk',
      );
    }
    ...
    return this.prisma.withdrawalRequest.create({
      data: {
        ...
        amount,
        status:
          risk.decision === RiskDecision.REVIEW
  ...
  async approveWithdrawal(requestId: string) {
    ...
    if (!request || request.status !== WithdrawalStatus.PENDING) {
      throw new BadRequestException(
        'Invalid withdrawal request or already processed',
      );
    }
  ...
  async transfer(
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (!fromWallet || fromWallet.balance < amount) {
        throw new BadRequestException(
          'Insufficient balance or source wallet not found',
        );
      }
      ...
      await tx.ledgerEntry.create({
        data: {
          ...
          description: description
            ? `Transfer to ${toWalletId}: ${description}`
      ...
      await tx.ledgerEntry.create({
        data: {
          ...
          referenceId,
          description: description
            ? `Transfer from ${fromWalletId}: ${description}`
