...
export function Offers({ token }: { token: string }) {
    ...
    const loadCoupons = async () => {
        const data = [
            { id: 'OFF-001', code: 'LOAN50', product: 'Vehicle Loan', discount: '$50 Cashback', type: 'Cashback', usage: 320, total: 1000, expiry: '2026-12-31', status: 'Active', color: 'blue' },
            { id: 'OFF-002', code: 'LOWINT', product: 'Business Loan', discount: '1.5% Interest Discount', type: 'Interest Rate', usage: 85, total: 200, expiry: '2026-06-30', status: 'Active', color: 'green' },
            { id: 'OFF-003', code: 'FREEFEE', product: 'Fuel Credit', discount: 'Fee Waiver', type: 'Fee Waiver', usage: 150, total: 500, expiry: '2026-03-31', status: 'Active', color: 'purple' },
        ];
        setCouponList(data);
    };
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '32px', paddingBottom: '32px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                    <Title level={4} style={{ margin: 0, fontWeight: 700 }}>Loan Incentives</Title>
                    <Text type="secondary" style={{ fontSize: '14px' }}>Strategic financial rewards to drive application volume and customer loyalty</Text>
                </div>
                <Button 
                    type="primary" 
                    icon={<Plus size={18} />} 
                            ...
                            <div style={{ 
                                ...
                                <Space direction="vertical" size={12} style={{ width: '100%' }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                        <Zap size={16} color="#faad14" fill="#faad14" />
                                        <Text strong style={{ fontSize: '16px', color: '#faad14' }}>{offer.discount}</Text>
                                    </div>
                ...
                <Row gutter={48} align="middle" style={{ width: '100%', zIndex: 1 }}>
                    <Col xs={24} md={18}>
                        <Space direction="vertical" size={12}>
                            ...
                            <Title level={3} style={{ color: 'white', margin: 0, fontWeight: 800 }}>Boost Loan Volume</Title>
                            <Text style={{ color: 'rgba(255,255,255,0.85)', fontSize: '16px', display: 'block', maxWidth: '600px' }}>
                                Financiers using dynamic interest rate incentives report a <strong>42% increase</strong> in customer lifetime value over standard products.
                            </Text>
                        </Space>
                    </Col>
