...
export class InvoicingService {
    ...
    async generateMerchantStatement(merchantId: string, startDate: Date, endDate: Date) {
        ...
        entries.forEach(entry => {
            if (entry.type === LedgerType.CREDIT && entry.description?.includes('Payout')) {
                totalEarnings += entry.amount;
            } else if (entry.type === LedgerType.DEBIT && entry.description?.toLowerCase().includes('withdrawal')) {
