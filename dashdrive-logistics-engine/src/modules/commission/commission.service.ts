...
export class CommissionService {
  ...
  async processOrderSettlement(orderId: string) {
    ...
    if (order.store.merchant.type === 'DIRECT') {
      ...
      const commissionRate = order.store.commissionRate || 0.1;
      ...
      let riderPayoutRate = 0.8; // Default for Food
      if (order.store.merchant.type === 'MART') riderPayoutRate = 0.7;

      riderEarned = deliveryFee * riderPayoutRate;
    }

    this.logger.log(
      `Settling Order #${order.orderNumber}: Platform(${platformCommission}), Merchant(${merchantEarned}), Rider(${riderEarned})`,
    );
