...
export class WebhooksController {
  ...
  @Post('payment')
  @ApiOperation({
    summary: 'Standardized internal webhook for payment confirmations',
  })
  async handlePaymentWebhook(@Body() body: any) {
    ...
    if (status === 'success') {
      updatedTransaction = await this.paymentsService.completePayment(
        transactionId,
  ...
  @Post('topup')
  @ApiOperation({
    summary: 'Standardized internal webhook for topup confirmations',
  })
  async handleTopupWebhook(@Body() body: any) {
    ...
    await this.paymentsService.completeTopUp(
      walletId,
      amount,
      gatewayTransactionId,
    );
