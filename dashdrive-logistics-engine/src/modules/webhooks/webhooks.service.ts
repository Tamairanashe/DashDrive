        ...
        for (const config of configs) {
            ...
            if (config.events.includes(event) || config.events.includes('*')) {
                ...
                if (!this.webhookQueue) {
                    this.logger.warn(`[MOCK] Webhook queue not available. Skipping ${event} for ${config.url}`);
                    continue;
                }
                ...
                this.logger.debug(`Queued webhook ${event} for merchant ${merchantId} to ${config.url}`);
            }
        }
    }

    async createConfig(merchantId: string, data: { url: string; events: string[] }) {
