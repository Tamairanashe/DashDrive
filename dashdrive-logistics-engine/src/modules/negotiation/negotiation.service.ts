...
import { OrderStatus, OfferStatus, DispatchAttemptStatus } from '@prisma/client';
...
export class NegotiationService {
    ...
    async createNegotiatedTrip(dto: CreateNegotiatedTripDto) {
        ...
        if (dto.rider_bid < quote.minimum_price) {
            throw new BadRequestException(`Bid too low. Minimum allowed price is ${quote.minimum_price}`);
        }
        ...
        if (!systemMerchant) {
            ...
            systemMerchant = await this.prisma.merchant.findFirst() || await this.prisma.merchant.create({
                data: {
        ...
        let systemStore = await this.prisma.store.findFirst({
            where: { merchantId: systemMerchant.id, name: `DashDrive ${dto.vertical}` }
        });
        ...
        this.logger.log(`Negotiated ${dto.vertical} trip created: ${order.orderNumber} with bid ${dto.rider_bid}`);

        // Broadcast to nearby drivers
        await this.broadcastNegotiatedTrip(order.id, dto.pickup_lat, dto.pickup_lng);
    ...
    async broadcastNegotiatedTrip(orderId: string, lat: number, lng: number) {
        ...
        this.logger.log(`Broadcasted trip ${order.orderNumber} to ${availableRiders.length} riders`);
    }

    async submitCounterOffer(riderId: string, offerId: string, bidAmount: number) {
        ...
        this.logger.log(`Rider ${riderId} countered with ${bidAmount} for offer ${offerId}`);
    ...
    async acceptNegotiatedOffer(orderId: string, bidId: string) {
        ...
        this.logger.log(`Ride ${orderId} accepted with rider ${riderId} at price ${finalPrice}`);
