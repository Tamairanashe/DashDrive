...
export class NegotiationController {
    ...
    @ApiResponse({ status: 201, description: 'Trip successfully created and broadcast initiated' })
    ...
    @Post('counter-offer')
    @ApiOperation({ summary: 'Submit a counter-offer for a trip request (Driver only)' })
    @ApiResponse({ status: 201, description: 'Counter-offer successfully submitted' })
    async counterOffer(@Req() req: any, @Body() dto: CounterOfferDto) {
        // In a real scenario, we would verify the user role is RIDER/DRIVER
        return this.negotiationService.submitCounterOffer(req.user.sub, dto.offerId, dto.bidAmount);
    }

    @Post('accept-bid/:orderId/:bidId')
    @ApiOperation({ summary: 'Accept a specific rider/driver bid (Rider/Customer choice)' })
