...
export class OrdersService {
  ...
  async updateStatus(
    ...
    if (status === OrderStatus.DELIVERED) {
      ...
      await this.cacheManager.del(
        `analytics:snapshot:${merchantId}:${order.storeId}`,
      );

      // === FINTECH SETTLEMENT (ZERO-SUM DOUBLE ENTRY) ===
      const commissionAmount = updatedOrder.subtotal * 0.1; // Fixed 10% commission
      const merchantPayout =
        updatedOrder.subtotal + updatedOrder.taxAmount - commissionAmount;

      try {
        // 1. Merchant Payout
        const merchantWallet = await this.walletService.getWallet(
        ...
        await this.walletService.credit(
          ...
          merchantPayout,
          updatedOrder.id,
          `Payout for Order #${updatedOrder.orderNumber}`,
        );
        ...
        const finalDeliveryFee =
          delivery?.deliveryFee || updatedOrder.deliveryFee;
        if (delivery && delivery.riderId && finalDeliveryFee > 0) {
          const riderWallet = await this.walletService.getWallet(
          ...
          await this.walletService.credit(
            ...
            finalDeliveryFee,
            updatedOrder.id,
            `Earnings for Order #${updatedOrder.orderNumber}`,
          );

          // Record in EarningsService for optimization
          await this.earningsService.recordEarnings(
            delivery.riderId,
            finalDeliveryFee,
          );
        ...
        await this.walletService.debit(
          escrowWallet.id,
          merchantPayout + finalDeliveryFee,
          updatedOrder.id,
          `Settlement for Order #${updatedOrder.orderNumber}`,
        );
      } catch (err) {
        console.error(
          `[Fintech] Settlement failed for Order ${updatedOrder.id}:`,
          err,
        );
      }
      ...
      await this.eventBusService.publish(
        PlatformEvent.ORDER_DELIVERED,
        updatedOrder,
      );
    } else if (status === OrderStatus.CONFIRMED) {
      // Publish Order Confirmed Event
      await this.eventBusService.publish(
        PlatformEvent.ORDER_CONFIRMED,
        updatedOrder,
      );
    } else {
  ...
  private async handleStatusPushNotifications(order: any) {
    ...
    if (title && body) {
      ...
      await Promise.all(
        devices.map((device) =>
          this.notificationsService.sendPush(device.pushToken!, title, body, {
            orderId: order.id,
            status: order.status,
          }),
        ),
