...
export class FraudService {
  ...
  async evaluate(
    ...
    if (
      eventType === RiskEventType.DELIVERY &&
      metadata?.distance &&
      ...
      if (speedKmph > 150) {
        riskScore += 80;
        reasons.push(
          `Impossible delivery speed detected: ${speedKmph.toFixed(1)} km/h (GPS Spoofing)`,
        );
      }
    }
    ...
    this.logger.log(
      `Risk Evaluation: ${eventType} by ${actorType} ${actorId} - Score: ${riskScore} - Decision: ${decision}`,
    );
    ...
    if (
      decision === RiskDecision.BLOCKED &&
      (actorType === RiskActorType.MERCHANT ||
        actorType === RiskActorType.RIDER)
    ) {
      ...
      this.logger.warn(
        `AUTOMATED ACTION: Wallet frozen for ${actorType} ${actorId} due to critical risk score`,
      );
    }
