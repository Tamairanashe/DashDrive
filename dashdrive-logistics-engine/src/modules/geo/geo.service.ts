...
export class GeoService implements OnModuleInit, OnModuleDestroy {
    ...
    async updateRiderLocation(riderId: string, latitude: number, longitude: number) {
        ...
        pipeline.set(`${this.RIDER_STATUS_PREFIX}${riderId}`, 'online', 'EX', this.HEARTBEAT_TIMEOUT);
        await pipeline.exec();
    }
    ...
    async setRiderStatus(riderId: string, isOnline: boolean) {
        if (isOnline) {
            await this.redis.set(`${this.RIDER_STATUS_PREFIX}${riderId}`, 'online', 'EX', this.HEARTBEAT_TIMEOUT);
        } else {
    ...
    async isRiderOnline(riderId: string): Promise<boolean> {
        const status = await this.redis.get(`${this.RIDER_STATUS_PREFIX}${riderId}`);
