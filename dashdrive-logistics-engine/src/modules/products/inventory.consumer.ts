...
export class InventoryConsumer {
  ...
  async handleOrderCreated(@Payload() data: any) {
    this.logger.log(
      `Received ${PlatformEvent.ORDER_CREATED} event for order ${data.orderNumber}`,
    );
    ...
    for (const item of data.items) {
      try {
        ...
        if (product.stock <= product.lowStockThreshold) {
          this.logger.warn(
            `Low stock alert for product ${product.name}: ${product.stock}`,
          );
        ...
        this.logger.error(
          `Failed to deduct stock for product ${item.productId}: ${error.message}`,
        );
