...
export class MobileWalletController {
    ...
    async getOverview(@Request() req: any) {
        ...
        const wallet = await this.walletService.getWallet('MERCHANT', merchantId, 'USD');

        // Count pending withdrawals
        const pendingWithdrawalsCount = await (this.walletService as any).prisma.withdrawalRequest.count({
    ...
    async getTransactions(@Request() req: any) {
        const summary = await this.walletService.getWalletSummary('MERCHANT', req.user.sub, 'USD');
    ...
    async withdraw(@Request() req: any, @Body() body: { amount: number }) {
        const wallet = await this.walletService.getWallet('MERCHANT', req.user.sub, 'USD');
