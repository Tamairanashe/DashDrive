...
export class AnalyticsConsumer {
    ...
    async handleOrderDelivered(@Payload() data: any) {
        this.logger.log(`Received ${PlatformEvent.ORDER_DELIVERED} event for order ${data.orderNumber}`);
        ...
        try {
            ...
            this.logger.log(`Successfully updated daily stats for store ${data.storeId}`);
        } catch (error) {
