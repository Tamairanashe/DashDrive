...
export class SurgeEngineService {
   ...
   */
  async calculateDemandMultiplier(
    lat: number,
    lng: number,
    radiusKm: number = 5,
  ): Promise<number> {
    ...
    try {
      const availableRidersCount = await this.geoService.countNearbyRiders(
        lat,
        lng,
        radiusKm,
      );
      ...
      this.logger.debug(
        `Demand Ratio: ${demandRatio.toFixed(2)} | Multiplier: ${dynamicMultiplier}`,
      );
      ...
      this.logger.error(
        `Failed to calculate surge: ${(error as Error).message}`,
      );
      return 1.0; // Safe fallback
    }
  }

  private async checkManualSurgeZones(
    lat: number,
    lng: number,
  ): Promise<number> {
