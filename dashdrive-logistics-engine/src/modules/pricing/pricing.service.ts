...
export class PricingService {
  ...
  async getDeliveryQuote(dto: PricingQuoteDto) {
    ...
    const { distanceKm, estimatedTimeMin } =
      await this.distanceCalculator.calculateRouteDistance(
    ...
    const distanceFee = distanceKm * rule.distanceFeeKm;
    const timeFee = estimatedTimeMin * rule.timeFeeMin;
    const serviceFee = rule.serviceFee;

    const totalBaseCost = baseFee + distanceFee + timeFee + serviceFee;

    // 4. Demand Multiplier (Dynamic Pricing)
    const surgeMultiplier = await this.surgeEngine.calculateDemandMultiplier(
      dto.pickup_lat,
      dto.pickup_lng,
    );
    ...
    if (vertical === 'FOOD') payoutRate = 0.8;
    else if (vertical === 'MART') payoutRate = 0.7;
    ...
    const distributableCost =
      Math.max(0, baseFee + distanceFee + timeFee) * surgeMultiplier;
  ...
  async getNegotiationQuote(dto: PricingQuoteDto) {
    ...
    const suggestedPrice =
      distanceKm * rule.distanceFeeKm +
      estimatedTimeMin * rule.timeFeeMin +
      rule.baseFee * surgeMultiplier;

    // Minimum Fare logic: DriverCost + PlatformCost
    // Assuming DriverCost is ~70% of base suggested, and PlatformCost is a minimum fee
    const minimumFare = Math.max(5.0, suggestedPrice * 0.6);
  ...
  private async getPricingRule(countryCode: string) {
    ...
    if (!rule) {
      this.logger.warn(
        `No pricing rule found for ${countryCode}, using default fallback`,
      );
      return {
        baseFee: 2.0,
        distanceFeeKm: 0.8,
