...
export class RoutingService {
    ...
    async optimizeBatch(riderId: string) {
        ...
        if (!rider || !rider.latitude || !rider.longitude) {
            this.logger.warn(`Cannot optimize route for rider ${riderId} - no location data`);
        ...
        while (unvisited.length > 0) {
            ...
            if (nextStop.status === DeliveryStatus.ASSIGNED || nextStop.status === DeliveryStatus.ACCEPTED) {
                currentLocation = { 
