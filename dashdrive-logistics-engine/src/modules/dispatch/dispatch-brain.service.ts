...
export class DispatchBrainService implements OnModuleInit {
  ...
  onModuleInit() {
    this.logger.log(
      `🚀 Dispatch Brain initialized at H3 Resolution ${this.H3_RESOLUTION}`,
    );
  }
  ...
  async analyzeCityGrid() {
    ...
    const cellStats = new Map<
      string,
      { drivers: string[]; requests: string[] }
    >();
   ...
   */
  private async processNudges(
    cellStats: Map<string, { drivers: string[]; requests: string[] }>,
  ) {
    for (const [cell, data] of cellStats.entries()) {
      ...
      if (demand > 0) {
        ...
        if (ratio < this.MIN_RATIO_THRESHOLD) {
          this.logger.log(
            `⚠️ Shortage detected in cell ${cell}: Ratio ${ratio.toFixed(2)} (${supply} drivers / ${demand} requests)`,
          );
        ...
        this.logger.log(
          `ℹ️ Over-supply in cell ${cell}: ${supply} idle drivers.`,
        );
    ...
    return {
      ...
      ratio:
        data.requests.length > 0
          ? data.drivers.length / data.requests.length
          : Infinity,
    };
  }
}
