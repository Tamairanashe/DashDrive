...
export class EarningsService {
    ...
    private readonly TARGET_HOURLY_INCOME = 15.00; // Target $15/hr
    ...
    async recordEarnings(riderId: string, amount: number) {
        ...
        let earningsLog: { t: number; a: number }[] = await this.cacheManager.get(key) || [];
        
        // Add new entry
        earningsLog.push({ t: timestamp, a: amount });
        
        // Clean up old entries outside the window
        earningsLog = earningsLog.filter(e => e.t > (timestamp - this.ROLLING_WINDOW_MS));
    ...
    async getRollingHourlyIncome(riderId: string): Promise<number> {
        const key = `rider:earnings:${riderId}`;
        const earningsLog: { t: number; a: number }[] = await this.cacheManager.get(key) || [];
        
        const timestamp = Date.now();
        const validEarnings = earningsLog.filter(e => e.t > (timestamp - this.ROLLING_WINDOW_MS));
     ...
     */
    async getEarningsPriorityScore(riderId: string, idleMinutes: number): Promise<number> {
        ...
        this.logger.debug(`Rider ${riderId}: Income $${currentIncome}, Idle ${idleMinutes}m -> Score ${totalScore.toFixed(2)}`);
