...
import {
  ...
  ShieldAlert, AlertTriangle, AlertCircle, Info, Send,
} from 'lucide-react';
...
const SEVERITY_CONFIG: Record<string, { icon: React.ElementType; color: string; bg: string; border: string }> = {
  ...
  severe:   { icon: AlertTriangle, color: 'text-orange-700', bg: 'bg-orange-50', border: 'border-orange-200' },
  warning:  { icon: AlertCircle, color: 'text-amber-700', bg: 'bg-amber-50', border: 'border-amber-200' },
...
const ACTION_CONFIG: Record<string, { icon: React.ElementType; color: string }> = {
  surge:       { icon: Zap, color: 'text-violet-600' },
  notify:      { icon: Bell, color: 'text-blue-600' },
  dispatch:    { icon: Send, color: 'text-green-600' },
  investigate: { icon: SearchIcon, color: 'text-amber-600' },
  incentive:   { icon: DollarSign, color: 'text-emerald-600' },
};
                  ...
                  <Card key={alert.id} className={`shadow-none border ${cfg.border} ${cfg.bg}`}>
                    <CardContent className="p-3">
                      <div className="flex items-start gap-2">
                        <div className={`p-1 rounded-full ${cfg.bg} border-2 border-white shadow-sm`}>
              ...
              {recommendedActions.slice(0, 4).map((action) => {
                ...
                return (
                  <div key={action.id} className="p-2.5 rounded-lg border border-slate-200 hover:border-slate-300 transition-colors">
                    <div className="flex items-start gap-2">
                      <div className="p-1 px-1.5 rounded bg-slate-100 border border-slate-200 shadow-sm">
                        <Icon className={`w-3.5 h-3.5 ${cfg.color}`} />
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="text-xs font-semibold text-slate-900 leading-tight">{action.title}</div>
                        <div className="text-[10px] text-slate-500 mt-1 line-clamp-2">{action.description}</div>
                        <div className="text-[10px] text-emerald-600 font-bold mt-1.5 uppercase tracking-tight">
                          Impact: {action.estimatedImpact}
                        </div>
                      </div>
                      <Badge variant="secondary" className={`text-[9px] h-4 px-1 shrink-0 ${action.priority === 'high' ? 'bg-red-50 text-red-700 border-red-100' : action.priority === 'medium' ? 'bg-amber-50 text-amber-700 border-amber-100' : 'bg-slate-50 text-slate-600 border-slate-100'}`}>
                        {action.priority}
                      </Badge>
                    </div>
                  </div>
