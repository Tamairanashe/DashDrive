...
export function buildKPIMetrics(zones: HeatMapZone[]): KPIMetric[] {
  ...
  const totalDemand = zones.reduce((s, z) => s + (z?.metrics?.activeDemand || 0), 0);
  const totalIdle = zones.reduce((s, z) => s + (z?.metrics?.idleSupply || 0), 0);
  const avgEta = Math.round((zones.reduce((s, z) => s + (z?.metrics?.avgETA || 0), 0) / zones.length) * 10) / 10;
  const avgCancel = Math.round((zones.reduce((s, z) => s + (z?.metrics?.cancelRate || 0), 0) / zones.length) * 10) / 10;
  const avgFulfill = Math.round((zones.reduce((s, z) => s + (z?.metrics?.fulfillmentRate || 0), 0) / zones.length) * 10) / 10;
  const surgeCount = zones.filter((z) => z?.metrics?.surgeMultiplier !== null && z?.metrics?.surgeMultiplier !== undefined).length;
  const totalRevenue = zones.reduce((s, z) => s + (z?.metrics?.revenue || 0), 0);
  const hottest = zones.reduce((a, b) => ((a?.metrics?.imbalanceScore || 0) > (b?.metrics?.imbalanceScore || 0) ? a : b));
