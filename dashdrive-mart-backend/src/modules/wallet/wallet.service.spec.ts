...
import { WalletOwnerType, LedgerType, RiskDecision, RiskEventType, RiskActorType } from '@prisma/client';
...
describe('WalletService', () => {
    ...
    describe('credit', () => {
        it('should credit funds and create ledger entry', async () => {
            const walletId = 'w1';
            mockPrisma.wallet.findUnique.mockResolvedValue({ id: walletId, balance: 100, isFrozen: false });
        ...
        it('should throw if wallet is frozen', async () => {
            mockPrisma.wallet.findUnique.mockResolvedValue({ id: 'w1', isFrozen: true });
            await expect(service.credit('w1', 50)).rejects.toThrow(BadRequestException);
        });
    });
    ...
    describe('requestWithdrawal', () => {
        it('should block withdrawal if fraud engine decides BLOCKED', async () => {
            ...
            await expect(service.requestWithdrawal('w1', 500)).rejects.toThrow(BadRequestException);
        });
